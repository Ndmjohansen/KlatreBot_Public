...
## Deploy to Linux host (one-shot)

Clone repo to `/path/to/repo`, install Poetry for the service user, then:
...
`install.sh` is idempotent and safe to re-run after pulling new commits. It installs dependencies, creates `service user data directory`, writes a placeholder env file if missing, installs the systemd unit, and registers backup cron.
...
CI auto-deploys on push to `main` only. The deploy job SSHes to the host, checks out `main`, runs `poetry install --sync`, rewrites `/etc/klatrebot/klatrebot.env` from GitHub secrets, installs the templated unit, and restarts `klatrebot`.
...
```
0 3 * * * bash /path/to/repo/backup/backup.sh /path/to/klatrebot_v2.db gdrive
```
